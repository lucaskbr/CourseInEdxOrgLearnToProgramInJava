...
/* ****Lucas Klasa 01/06/2018
 * ****Codigo destinado apenas ao estudo. 
 * 	   Parte do codigo já veio pronto para utilização, apenas incrementei alguns metodos para funcionar como proposto na atividade.
 * 	   Atividade do curso: Learn to Program in Java -  Project Crypto - Edx.org */
...
public class Maze {
	...
	/*Solicita uma direção para o usuário, verifica se é uma possível, se sim faz a ação*/
	public String userMove() {
		...
		if(answer.equals("R") & canIMoveRight()) {
			moveRight();	
			printMap(); 
		}
		else if(answer.equals("L") & canIMoveLeft()) {
			moveLeft();	
			printMap();
		}
		else if(answer.equals("U") & canIMoveUp()) {
			moveUp();	
			printMap();
		}
		else if(answer.equals("D") & canIMoveDown()) {
			moveDown();	
			printMap();
		}
		else {
	...
	/*Verifica se existe um "buraco" no game se tiver pergunta seo usuario deseja pular*/
	public void navigatePit(String answer) {

		Scanner input = new Scanner(System.in);
		String decision = "";

		if(isThereAPit(answer)) {
	...
	public void movesMessage(){
		...
		if(counter_moves == 100) {
			System.out.println("Oh no! You took too long to escape, and now the maze exit is closed FOREVER >:");
		}
	}
	
	/*Repete os metodos principais ate certo numero de jogadas ou se o jogador chegar ao destino final*/
	...
	public void gameOn() {
		...
		}while(!didIWin() && counter_moves!= 100);
